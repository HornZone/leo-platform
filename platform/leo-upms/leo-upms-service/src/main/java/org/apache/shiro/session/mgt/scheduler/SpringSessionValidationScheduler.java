...
/**
 * 使用spring的任务调度器完成 session验证 很多用户登录后，会保存他们的session，用于记录用户登陆状态等信息， 这个任务调度器用于对session进行验证，查看是否过期或者停止，
 * 具体验证内容可以查看onlinewebsessionmanager中的validateSessions方法 如果验证没通过，则就需要进行某些操作 功能直接复制了
 * {@link org.apache.shiro.session.mgt.quartz.QuartzSessionValidationScheduler}
 * <p>
 * Title:
 * </p>
 * <p>
    ...
    public void enableSessionValidation() {
        ...
        try {
            ...
            if (log.isErrorEnabled()) {
                log.error(
                    "Error starting the Spring Scheduler session validation job.  Session validation may not occur.", e);
            }
        }
    }
