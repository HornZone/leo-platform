...
import com.leo.platform.pay.account.enums.AccountFundDirectionEnum;
 ...
 * 账户操作service实现类
 */
@Service("lpAccountTransactionService")
public class LpAccountTransactionServiceImpl implements LpAccountTransactionService {

    private static final Log LOG = LogFactory.getLog(LpAccountTransactionServiceImpl.class);
     ...
     * @return
     */
    private LpAccount getByUserNo_IsPessimist(String userNo, boolean isPessimist) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("userNo", userNo);
        map.put("isPessimist", isPessimist);
        return lpAccountDao.getByUserNo(map);
    }
    ...
    public LpAccount creditToAccount(String userNo, BigDecimal amount, String requestNo, String trxType, String remark) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public LpAccount creditToAccount(String userNo, BigDecimal amount, String requestNo, String bankTrxNo,
        String trxType, String remark) {
        LpAccount account = this.getByUserNo_IsPessimist(userNo, true);
        if (account == null) {
        ...
        // 记录账户历史
        LpAccountHistory accountHistoryEntity = new LpAccountHistory();
        ...
        this.lpAccountHistoryDao.insert(accountHistoryEntity);
        this.lpAccountDao.update(account);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public LpAccount debitToAccount(String userNo, BigDecimal amount, String requestNo, String trxType, String remark) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public LpAccount debitToAccount(String userNo, BigDecimal amount, String requestNo, String bankTrxNo,
        String trxType, String remark) {
        LpAccount account = this.getByUserNo_IsPessimist(userNo, true);
        if (account == null) {
            throw AccountBizException.ACCOUNT_NOT_EXIT;
    ...
    @Transactional(rollbackFor = Exception.class)
    public LpAccount freezeAmount(String userNo, BigDecimal freezeAmount) {
        LpAccount account = this.getByUserNo_IsPessimist(userNo, true);
        if (account == null) {
            throw AccountBizException.ACCOUNT_NOT_EXIT;
        }
    ...
    @Transactional(rollbackFor = Exception.class)
    public LpAccount unFreezeAmount(String userNo, BigDecimal amount, String requestNo, String trxType, String remark) {
        LpAccount account = this.getByUserNo_IsPessimist(userNo, true);
        if (account == null) {
            throw AccountBizException.ACCOUNT_NOT_EXIT;
        }
    ...
    @Transactional(rollbackFor = Exception.class)
    public LpAccount unFreezeSettAmount(String userNo, BigDecimal amount) {
        LpAccount account = this.getByUserNo_IsPessimist(userNo, true);
        if (account == null) {
            throw AccountBizException.ACCOUNT_NOT_EXIT;
        }
    ...
    public void settCollectSuccess(String userNo, String collectDate, int riskDay, BigDecimal totalAmount) {
        ...
        lpAccountHistoryDao.updateCompleteSettTo100(params);
