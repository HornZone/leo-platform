...
import com.leo.platform.pay.user.dao.BuildNoDao;
...
public class BuildNoServiceImpl implements BuildNoService {
    ...
    public String buildAccountNo() {
        ...
        String dateString = DateUtils.toString(new Date(), "yyyyMMdd");
        String accountNo =
            ACCOUNT_NO_PREFIX + dateString + accountNoSeq.substring(accountNoSeq.length() - 8, accountNoSeq.length());
    ...
    public String buildBankOrderNo() {
        ...
        String dateString = DateUtils.toString(new Date(), "yyyyMMdd");
        String bankOrderNo =
            BANK_ORDER_NO_PREFIX + dateString
                + bankOrderNoSeq.substring(bankOrderNoSeq.length() - 8, bankOrderNoSeq.length());
        return bankOrderNo;
    }
    ...
    public String buildReconciliationNo() {
        ...
        String dateString = DateUtils.toString(new Date(), "yyyyMMdd");
        String batchNo =
            RECONCILIATION_BATCH_NO + dateString + batchNoSeq.substring(batchNoSeq.length() - 8, batchNoSeq.length());
        return batchNo;
    }
